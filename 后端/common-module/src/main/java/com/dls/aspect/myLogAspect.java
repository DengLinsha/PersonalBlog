...
public class myLogAspect {
    ...
    @Pointcut("@annotation(com.dls.annotation.mySystemlog)")
    // 1.确定哪个切点，以后哪个类想成为切点，就在哪个类添加上面那行的注解。例如下面这个xxpt()就是切点
    public void pt(){

    }

    // 2.定义通知的方法(这里用的是环绕通知)，通知的方法也就是增强的具体代码。@Around注解表示该通知的方法会用在哪个切点
    @Around("pt()")
    ...
    private void handleBefore(ProceedingJoinPoint joinPoint) {
        ...
        // 打印请求入参。JSON.toJSONString、FastJson提供的工具方法，能把数组转成JSON
